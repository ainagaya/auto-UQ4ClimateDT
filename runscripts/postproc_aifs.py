...
def define_variables(config):
    ...
    return model_checkpoint, INI_DATA_PATH, start_time, end_time, data_inner_steps, inner_steps, plots_path, members, output_path, output_format

def read_gribfile(grib_file):
    """
    Convert a GRIB file to Zarr format.
    """
    ds = xr.open_dataset(grib_file, engine='cfgrib')
    return ds

def read_zarrfile(zarr_file):
    """
    Read a Zarr file.
    """
...
def read_netcdffile(netcdf_file):
    """
    Read a NetCDF file.
    """
...
model_checkpoint, INI_DATA_PATH, start_time, end_time, data_inner_steps, inner_steps, plots_path, members, output_path, output_format = define_variables(config)
    ...
    for folder in folders:
        ...
        for file_path in file_list:
            if output_format == "grib":
                ds = read_gribfile(file_path)
            elif output_format == "netcdf":
                ds = read_netcdffile(file_path)
            elif output_format == "zarr":
                ds = read_zarrfile(file_path)
